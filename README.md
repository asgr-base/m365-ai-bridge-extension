...
Claude Code (AI)
  ↕  MCP stdio（JSON-RPC）
native/mcp-server.js  ─── MCP Tools: teams_read_messages / teams_queue_reply / teams_get_status / teams_download_file
...
teams_download_file(filename="report.xlsx")  # 添付ファイルをダウンロード
```
...
| `teams_download_file` | `filename` (string), `groupId?`, `chatType?`, `downloadDir?` | Teams メッセージの添付ファイルを Graph API 経由でダウンロード |
...
| POST | `/token` | Extension から Graph API トークンを受信 |
...
- `content-script-dm.test.js`: DM メッセージ読み取り 7件
- `content-script-attachments.test.js`: ファイル添付抽出 5件
- `content-script-inspect.test.js`: DOM構造調査 4件
- `content-script-token.test.js`: Graph トークン取得 2件
- `mcp-server.test.js`: MCP ツール呼び出し 9件
- `mcp-server-download.test.js`: ファイルダウンロード 4件
- `popup.test.js`: ポップアップ UI 10件
...
- [x] ファイル添付名の抽出（`file-preview-root` セレクタ対応）
- [x] Graph API 経由のファイルダウンロード（`teams_download_file` MCP ツール）
- [ ] 複数チャンネル・チャットの切り替え対応
...
- ネットワーク外部からのアクセス不可
- Graph API トークンは Teams ブラウザの MSAL キャッシュから取得し、ローカル MCP サーバーにのみ送信（外部送信なし）
- トークンはインメモリ保持のみ（ファイル保存なし）。サーバー停止で消失
- ダウンロードファイルは `downloads/` に保存（`.gitignore` で git 管理外）
